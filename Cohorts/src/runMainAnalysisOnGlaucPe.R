...
print("Running analysis restricted to those with POAG with peripheral visual field loss - this will take a few minutes ...")

load("data/mets_eset_with_covars_by_subtype_and_residuals.RData")
source("./src/myRegressions.R")

fdata = fData(mets.glaucpe.m1)
pdata = pData(mets.glaucpe.m1)
...
# model 3: set 1 of established risk factors for exf glaucoma

uclog.reg.res.m3 = runRegression(regression.type = "unconditional.logistic.regression",
                                my.eset = mets.glaucpe.m3,
                                outcome = "glaupe", 
                                covariates =c("ageyr",#"fast","menobld","menodiag", "bldtime", "bldmon","cohort",
                                              #"hrace","smokec", 
                                              "ibmils", "acts",
...
uclog.reg.res.m5 = runRegression(regression.type = "unconditional.logistic.regression",
                                ...
                                covariates = c("ageyr",#"fast","menobld","menodiag", "bldtime", "bldmon","cohort",
                                               ...
                                               "nses"  ,"agemeno",
                                               "nitras", "alcos", "caffs", "aheis" ,"calors"),
                                               #"hbph","cholh","sdbh", "eversteroid"),
                                strata = NULL, allow.na.values.in.exposure = T,
                                file.with.result.prefix = file.path(main.dir, 
                                                                    "/results/uclog_glau_pe_m5"),
                                store.complete.result = T)

or.table.m5= calculateORCI(result.data = uclog.reg.res.m5, or.scale = 1, file.with.result.prefix =
                              file.path(main.dir, "/results/uclog_glau_pe_m5"),
                            estimate = "estimate", l.ci = "l.ci", u.ci = "u.ci", exp.ci = T, estimate.CI = T)
...
save.image(file = "./results/image_runMainAnalysisOnPOAG-Pe.RData")
#load("./results/image_runMainAnalysisOnPOAG-Pe.RData")
...
res.all = res.all.h4
write.table(res.all, file = "./results/main_analysis_all_models_poag_pe.csv", sep = "\t", row.names = F, col.names = T, quote = F)
...
#nes$stars[which(pfdr$value<=0.05)] = "**"
nes$stars[which(pnef$value<=0.2)] = "+"
#nes$stars[which(pnef$value<=0.05)] = "++"
...
range(nes$value)
ggplot(data = nes, aes(x = variable, y = Metabolite, fill = value)) + #data and lables
  geom_tile(color = "white", size = 0.1) + # what to plot
  #scale_fill_continuous(colours = hm.palette)+
  scale_fill_gradientn(colours = hm.palette, limits = c(0,2.1))+
  geom_text(aes(x = variable, y = Metabolite, label=stars), color="black", size=5, 
            nudge_x = 0, nudge_y = 0, fontface = "bold") +
  theme_minimal()+ # minimal theme
  ggtitle("POAG with peripheral visual field loss")+
...
ggsave(filename=file.path("./results/heatmap_sig_metabolites_main_all_models_poag_pe.png"), device = "png", dpi = 600, height = 8, width = 10)  
