...
print("Running Metabolite Set Enrichment Analysis ...")
...
res.m1.annot.new[which(is.na(res.m1.annot.new$class)), c("METABOLITE","class")]

# number of metabolite classes
length(unique(res.m1.annot.new$class))
my.clsses = table(res.m1.annot.new$class)
my.clsses[which(my.clsses>=3)]
length(my.clsses[which(my.clsses>=3)])

res.all.file = res.m1.annot.new[,c(1,2,19,3:18)] 
res.all.file[,c(grep(pattern = "p.value",x = colnames(res.all.file),fixed = T),
                grep(pattern = "NEF",x = colnames(res.all.file),fixed = T))] = 
           round(res.all.file[,c(grep(pattern = "p.value",x = colnames(res.all.file),fixed = T),
                                 grep(pattern = "NEF",x = colnames(res.all.file),fixed = T))],2)

# on purpose over-writing this file, to add the same data but with metabolite classes included
# metabolites not assigned to a class
write.table(res.all.file, file = "./results/main_analysis_all_models.csv", sep = "\t", row.names = F, col.names = T, quote = F)
...
set.seed(1429)
# model 1
load("./results/clog_all_m1_table_raw.RData")
res.m1 = result.data
m1.mets = res.m1$estimate
names(m1.mets) = res.m1$METABOLITE
m1.mets = m1.mets[which(!is.na(m1.mets))]
m1.fgsea = runFGSEA(pathways = met.classes, stats = m1.mets, file.result = "./results/gsea_m1.csv")

# model 2
load("./results/clog_all_m2_table_raw.RData")
res.m2 = result.data 
m2.mets = res.m2$estimate
names(m2.mets) = res.m2$METABOLITE
m2.mets = m2.mets[which(!is.na(m2.mets))]
m2.fgsea = runFGSEA(pathways = met.classes, stats = m2.mets, file.result = "./results/gsea_m2.csv")

# model 3
load("./results/clog_all_m3_table_raw.RData")
res.m3 = result.data
m3.mets = res.m3$estimate
names(m3.mets) = res.m3$METABOLITE
m3.mets = m3.mets[which(!is.na(m3.mets))]
m3.fgsea = runFGSEA(pathways = met.classes, stats = m3.mets, file.result = "./results/gsea_m3.csv")

# model 4
load("./results/clog_all_m4_table_raw.RData")
res.m4 = result.data
m4.mets = res.m4$estimate
names(m4.mets) = res.m4$METABOLITE
m4.mets = m4.mets[which(!is.na(m4.mets))]
m4.fgsea = runFGSEA(pathways = met.classes, stats = m4.mets, file.result = "./results/gsea_m4.csv")
...
m5.fgsea = runFGSEA(pathways = met.classes, stats = m5.mets, file.result = "./results/gsea_m5.csv")
...
write.table(fgsea.res, file = "./results/fgsea_main_all_models.csv", row.names = F, col.names = T, quote = F, sep = "\t")
...
my.order.sig.m4 = m5.fgsea$pathway[which(m5.fgsea$padj<0.2)]
save(my.order,my.order.sig.m4, file = "./results/metabolite_classes_plot_order.RData")
...
range(nes$value)
ggp<-ggplot(data = nes, aes(x = variable, y = Pathway, fill = value)) + #data and lables
...
ggsave(filename=file.path("./results/heatmap_GSEA_broad_main_all_models.png"), 
       plot = ggp, device = "png", dpi = 600, width = 10, height = 6)  

ggsave(filename=file.path("./results/heatmap_GSEA_broad_main_all_models.svg"), 
       plot = ggp, device = "svg", dpi = 600, width = 10, height = 6)  

print("Figure: ./results/heatmap_GSEA_broad_main_all_models.png")
print("Done")
