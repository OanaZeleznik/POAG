...
print("Running analyses stratified by time between blood collection and diagnosis ...")
...
nhs.clog.reg.res = runRegression(regression.type = "conditional.logistic.regression",
                                 ...
                                 strata = "matchid", allow.na.values.in.exposure = T,
                                 file.with.result.prefix = file.path(main.dir, 
                                                                     "/results/clog_closeDX_m5"),
                                 store.complete.result = T)

nhs.or.table = calculateORCI(result.data = nhs.clog.reg.res, or.scale = 1, file.with.result.prefix =
                               file.path(main.dir, "/results/clog_closeDX_m5"),
                             estimate = "estimate", l.ci = "l.ci", u.ci = "u.ci", exp.ci = F, estimate.CI = F)
...
#### HPFS
hpfs.clog.reg.res = runRegression(regression.type = "conditional.logistic.regression",
                                  my.eset = farDX.glauc.m5,
                                  outcome = "caco", 
                                  covariates =  c("ageyr", 
                                                  "ibmils", "acts", #"bldtime", "bldmon", 
                                                  "nses", 
                                                  "nitras", "alcos", "caffs", "aheis" ,"calors"),                                                  
                                  strata = "matchid", allow.na.values.in.exposure = T,
                                  file.with.result.prefix = file.path(main.dir, 
                                                                      "/results/clog_farDX_m5"),
                                  store.complete.result = T)

hpfs.or.table = calculateORCI(result.data = hpfs.clog.reg.res, or.scale = 1, file.with.result.prefix =
                                file.path(main.dir, "/results/clog_farDX_m5"),
                              estimate = "estimate", l.ci = "l.ci", u.ci = "u.ci", exp.ci = F, estimate.CI = F)
hpfs.clog.reg.res = hpfs.clog.reg.res[rownames(hpfs.or.table),]
all.equal(hpfs.clog.reg.res$METABOLITE, hpfs.or.table$METABOLITE)
hpfs.or.table$p.value = hpfs.clog.reg.res$p.value
...
save.image("./results/image_runMainAnalysisByTimeBloodDX.RData")
##############################################################
library(ggplot2)
#load("./results/image_runMainAnalysisByTimeBloodDX.RData")
...
res1.m1$strata = "DX<10.17y"
res1.m1$model = "Model 1"

res2.m1$strata = "DX>=10.17y"
res2.m1$model = "Model 1"

res1.m4$strata = "DX<10.17y"
res1.m4$model = "Model 5"

res2.m4$strata = "DX>=10.17y"
...
ggplot(data = res.sig, aes(x = strata, y = METABOLITE, fill = OR)) + #data and lables
  ...
  facet_wrap(.~model)+
  scale_fill_gradientn(colours = hm.palette, limits = c(0.3,1.7), breaks = c(0.5, 1, 1.5))+
...
ggsave(filename=file.path("./results/heatmap_sig_metabolites_by_TimeDX_all_models.png"), 
       device = "png", dpi = 600, width = 10, height = 9)  

print("Figure: ./results/heatmap_sig_metabolites_by_TimeDX_all_models.png")
...
colnames(res1.m1) = paste(colnames(res1.m1),".closeDX.m1", sep ="")
rownames(res1.m4) = res1.m4$METABOLITE
colnames(res1.m4) = paste(colnames(res1.m4),".closeDX.m5", sep ="")

rownames(res2.m1) = res2.m1$METABOLITE
colnames(res2.m1) = paste(colnames(res2.m1),".farDX.m1", sep ="")
rownames(res2.m4) = res2.m4$METABOLITE
colnames(res2.m4) = paste(colnames(res2.m4),".farDX.m5", sep ="")
...
write.table(res.all, file = "./results/analysis_by_TimeBloodDX_all_models.csv", sep = "\t", row.names = F, col.names = T, quote = F)
