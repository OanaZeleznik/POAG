...
# model 1
load("./results/uclog_glauc_pa_m1_table_raw.RData")
res.m1 = result.data
m1.mets = res.m1$estimate
names(m1.mets) = res.m1$METABOLITE
m1.mets = m1.mets[which(!is.na(m1.mets))]
m1.fgsea = runFGSEA(pathways = met.classes, stats = m1.mets, file.result = "./results/gsea_poag_pa_m1.csv")

# model 2
load("./results/uclog_glau_pa_m2_table_raw.RData")
res.m2 = result.data 
m2.mets = res.m2$estimate
names(m2.mets) = res.m2$METABOLITE
m2.mets = m2.mets[which(!is.na(m2.mets))]
m2.fgsea = runFGSEA(pathways = met.classes, stats = m2.mets, file.result = "./results/gsea_poag_pa_m2.csv")

# model 3
load("./results/uclog_glau_pa_m3_table_raw.RData")
res.m3 = result.data
m3.mets = res.m3$estimate
names(m3.mets) = res.m3$METABOLITE
m3.mets = m3.mets[which(!is.na(m3.mets))]
m3.fgsea = runFGSEA(pathways = met.classes, stats = m3.mets, file.result = "./results/gsea_poag_pa_m3.csv")

# model 4
load("./results/uclog_glau_pa_m4_table_raw.RData")
res.m4 = result.data
m4.mets = res.m4$estimate
names(m4.mets) = res.m4$METABOLITE
m4.mets = m4.mets[which(!is.na(m4.mets))]
m4.fgsea = runFGSEA(pathways = met.classes, stats = m4.mets, file.result = "./results/gsea_poag_pa_m4.csv")

# model 5
load("./results/uclog_glau_pa_m5_table_raw.RData")
res.m5 = result.data
m5.mets = res.m5$estimate
names(m5.mets) = res.m5$METABOLITE
m5.mets = m5.mets[which(!is.na(m5.mets))]
m5.fgsea = runFGSEA(pathways = met.classes, stats = m5.mets, file.result = "./results/gsea_poag_pa_m5.csv")
...
write.table(fgsea.res, file = "./results/fgsea_main_poag_pa_models.csv", row.names = F, col.names = T, quote = F, sep = "\t")
...
gg <- ggplot(data = nes, aes(x = variable, y = Pathway, fill = value)) + #data and lables
  ...
  ggtitle("A.     POAG with paracentral visual field loss")+
...
ggsave(filename=file.path("./results/heatmap_GSEA_broad_main_poag_pa_models.png"), device = "png", 
       dpi = 600, width = 10, height = 6, plot = gg)  

ggsave(filename=file.path("./results/heatmap_GSEA_broad_main_poag_pa_models.svg"), device = "svg", 
       dpi = 600, width = 10, height = 6, plot = gg)  

print("Figure: ./results/heatmap_GSEA_broad_main_poag_pa_models.png")
print("Done")
