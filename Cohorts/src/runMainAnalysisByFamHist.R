...
nhs.or.table = calculateORCI(result.data = nhs.clog.reg.res, or.scale = 1, file.with.result.prefix =
                               file.path(main.dir, "/results/log_hist_m1"),
...
hpfs.clog.reg.res = runRegression(regression.type = "unconditional.logistic.regression",
                                  my.eset = nfh.glauc.m1,
                                  outcome = "caco", 
                                  covariates = c("ageyr"),#"fast", "menobld","menodiag","cohort","bldtime", "bldmon","hrace"),
                                  strata = NULL, allow.na.values.in.exposure = T,
                                  file.with.result.prefix = file.path(main.dir, 
                                                                      "/results/log_nohist_m1"),
                                  store.complete.result = T)

hpfs.or.table = calculateORCI(result.data = hpfs.clog.reg.res, or.scale = 1, file.with.result.prefix =
                               file.path(main.dir, "/results/log_nohist_m1"),
...
nhs.or.table = calculateORCI(result.data = nhs.clog.reg.res, or.scale = 1, file.with.result.prefix =
                               file.path(main.dir, "/results/log_hist_m5"),
                             estimate = "estimate", l.ci = "l.ci", u.ci = "u.ci", exp.ci = T, estimate.CI = T)
...
hpfs.clog.reg.res = runRegression(regression.type = "unconditional.logistic.regression",
                                  ...
                                  strata = NULL, allow.na.values.in.exposure = T,
                                  file.with.result.prefix = file.path(main.dir, 
                                                                      "/results/log_nohist_m5"),
                                  store.complete.result = T)

hpfs.or.table = calculateORCI(result.data = hpfs.clog.reg.res, or.scale = 1, file.with.result.prefix =
                                file.path(main.dir, "/results/log_nohist_m5"),
                              estimate = "estimate", l.ci = "l.ci", u.ci = "u.ci", exp.ci = T, estimate.CI = F)
hpfs.clog.reg.res = hpfs.clog.reg.res[rownames(hpfs.or.table),]
all.equal(hpfs.clog.reg.res$METABOLITE, hpfs.or.table$METABOLITE)
hpfs.or.table$p.value = hpfs.clog.reg.res$p.value
...
save.image("./results/image_runMainAnalysisFamHist.RData")
##############################################################
library(ggplot2)
#load("./results/image_runMainAnalysisFamHist.RData")
...
res1.m1$strata = "POAG History"
res1.m1$model = "Model 1"

res2.m1$strata = "No POAG History"
res2.m1$model = "Model 1"

res1.m4$strata = "POAG History"
res1.m4$model = "Model 5"

res2.m4$strata = "No POAG History"
...
ggplot(data = res.sig, aes(x = strata, y = METABOLITE, fill = OR)) + #data and lables
  ...
  facet_wrap(.~model)+
  scale_fill_gradientn(colours = hm.palette, limits = c(0,2.01), breaks = c(0.5, 1, 1.5))+
...
ggsave(filename=file.path("./results/heatmap_sig_metabolites_by_FamhHist_all_models.png"), 
       device = "png", dpi = 600, width = 11, height = 10)  

print("Figure: ./results/heatmap_sig_metabolites_by_FamhHist_all_models.png")
...
colnames(res1.m1) = paste(colnames(res1.m1),".fam.hist.m1", sep ="")
rownames(res1.m4) = res1.m4$METABOLITE
colnames(res1.m4) = paste(colnames(res1.m4),".fam.hist.m5", sep ="")

rownames(res2.m1) = res2.m1$METABOLITE
colnames(res2.m1) = paste(colnames(res2.m1),".no.fam.hist.m1", sep ="")
rownames(res2.m4) = res2.m4$METABOLITE
colnames(res2.m4) = paste(colnames(res2.m4),".no.fam.hist.m5", sep ="")
...
write.table(res.all, file = "./results/analysis_by_FamHist_all_models.csv", sep = "\t", row.names = F, col.names = T, quote = F)
