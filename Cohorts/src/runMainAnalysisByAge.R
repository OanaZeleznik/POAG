...
print("Running analyses stratified by age ...")
...
nhs.clog.reg.res = runRegression(regression.type = "unconditional.logistic.regression",
                                 ...
                                 strata = NULL, allow.na.values.in.exposure = T,
                                 file.with.result.prefix = file.path(main.dir, 
                                                                     "/results/log_lowAge_m5"),
                                 store.complete.result = T)

nhs.or.table = calculateORCI(result.data = nhs.clog.reg.res, or.scale = 1, file.with.result.prefix =
                               file.path(main.dir, "/results/log_lowAge_m5"),
...
hpfs.clog.reg.res = runRegression(regression.type = "unconditional.logistic.regression",
                                  ...
                                  strata = NULL, allow.na.values.in.exposure = T,
                                  file.with.result.prefix = file.path(main.dir, 
                                                                      "/results/log_highAge_m5"),
                                  store.complete.result = T)

hpfs.or.table = calculateORCI(result.data = hpfs.clog.reg.res, or.scale = 1, file.with.result.prefix =
                                file.path(main.dir, "/results/log_highAge_m5"),
                              estimate = "estimate", l.ci = "l.ci", u.ci = "u.ci", exp.ci = T, estimate.CI = F)
hpfs.clog.reg.res = hpfs.clog.reg.res[rownames(hpfs.or.table),]
all.equal(hpfs.clog.reg.res$METABOLITE, hpfs.or.table$METABOLITE)
hpfs.or.table$p.value = hpfs.clog.reg.res$p.value
...
save.image("./results/image_runMainAnalysisByAge.RData")
library(ggplot2)
...
res1.m1$strata = "Age<58.5y"
res1.m1$model = "Model 1"

res2.m1$strata = "Age>=58.5y"
res2.m1$model = "Model 1"

res1.m4$strata = "Age<58.5y"
res1.m4$model = "Model 5"

res2.m4$strata = "Age>=58.5y"
...
ggplot(data = res.sig, aes(x = strata, y = METABOLITE, fill = OR)) + #data and lables
  ...
  labs(caption = "* p<0.05; ** NEF-p<0.02; *** NEF-p<0.05 ")

ggsave(filename=file.path("./results/heatmap_sig_metabolites_by_Age_all_models.png"), device = "png", 
       dpi = 600, width = 10, height = 10)  
print("Figure: ./results/heatmap_sig_metabolites_by_Age_all_models.png")
...
colnames(res1.m1) = paste(colnames(res1.m1),".age<58.5y.m1", sep ="")
rownames(res1.m4) = res1.m4$METABOLITE
colnames(res1.m4) = paste(colnames(res1.m4),".age<58.5y.m5", sep ="")

rownames(res2.m1) = res2.m1$METABOLITE
colnames(res2.m1) = paste(colnames(res2.m1),".age>=58.5y.m1", sep ="")
rownames(res2.m4) = res2.m4$METABOLITE
colnames(res2.m4) = paste(colnames(res2.m4),".age>=58.5y.m5", sep ="")
...
write.table(res.all, file = "./results/analysis_by_Age_all_models.csv", sep = "\t", row.names = F, col.names = T, quote = F)
print("Done")
